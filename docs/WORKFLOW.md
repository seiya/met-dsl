...
- 親 `node` は、直下依存 `node` がすべて `pass` または `xfail` になるまで `Judge` を開始してはならない。
...
### 7-2) ID と不変条件
- `plan_id` は `case.resolved.yaml` と `impl.resolved.yaml` と `dependency.resolved.yaml` の組を一意に識別する `ID` とする。推奨形式は `<spec_id>_<case_hash12>_<impl_hash12>` とする。
...
- `Judge` 開始条件は「対象 `node_key` の直下依存 `node` が `pass` または `xfail`」である。未達時は `blocked` を返す。
