...
- `plan_id` / `pipeline_id` の形式は `<slug>_<YYYYMMDD>_<seq3>` (slug は hyphen-separated lowercase alphanumeric)。例: `flux-rsn-p0_20260425_001`。slug 中の underscore は invalid。
...
| `reserve-phase-root` | plan_id / pipeline_id の予約 (path 実体化はしない) | [reserve-phase-root](#reserve-phase-root) |
...
| `step` | yes | `plan` / `generate` / `build` / `execute` / `judge` / `tune` / `promote` |
| `substep` | substep agent では yes | `generate` / `verify` |
| `orchestration_id` | yes | |
| `agent_run_id` | yes | child_agent_run_id と一致 |
| `parent_agent_run_id` | yes | |
| `workflow_mode` | yes | `dev` / `prod` |
| `plan_ref` | yes | `workspace/plans/<node_key_safe>/<plan_id>` (Plan phase 含めて全 phase で必須) |
| `pipeline_ref` | yes | `workspace/pipelines/<node_key_safe>/<pipeline_id>` (Plan phase でも必須。未生成なら先に `reserve-phase-root --step generate` で予約) |
| `dependency_ref` | yes | Plan: `spec/.../deps.yaml`、Generate 以降: workspace 内 phase root |
...
| `execution_id` | execute step では yes | 実行 ID (1 launch につき 1 つ pin) |
| `generation_id` | generate substep / execute / build (cross-phase Make) では yes | gen_id |
| `source_build_id` | execute step では yes | 使用する build_id |
...
| `--reason-code` | no | snake_case 識別子 (例: `plan_verify_shape_expr_invalid`) |
...
| `validation_stage` | generate/build/execute/judge の terminal status では yes | `post_generate` / `post_build` / `post_execute` / `pre_judge` / `full` のいずれか (step 別の許容値あり) |

---

## reserve-phase-root

`plan_id` または `pipeline_id` を予約。実 directory は作成しない (子 agent が作成する)。Plan phase の launch 前に `--step plan` (plan_id 予約) と `--step generate` (pipeline_id 予約) の両方が必要。
...
| `--orchestration-id` | yes | |
| `--node-key` | yes | |
| `--step` | yes | `plan` で plan_id 予約、`generate` で pipeline_id 予約 |
...
| `--step` | yes | `plan` / `generate` / `build` / `execute` / `judge` / `tune` / `promote` |
| `--backend` | no | 既定 `codex` |
| `--require-child-agent` | yes | `step` または `substep`。Plan/Generate/Tune は `substep`、Build/Execute/Judge/Promote は `step` |
...
| `validate_pipeline_semantics` | `{"stage": "plan|post_generate|post_build|post_execute|pre_judge|full", "plan_ref": "workspace/plans/..." (plan stage), "pipeline_root": "workspace/pipelines/..." または list, "generation_id": "<id>" (optional)}` |
...
| `--paths-json` | yes | JSON list of path strings (例: `'["workspace/plans/.../plan_meta.json"]'`) |
