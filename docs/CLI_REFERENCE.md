...
| `substep` | yes for a substep agent | Compile: `generate` / `verify`. Generate: `generate` / `lint` / `static` / `verify` (`lint` and `static` are conductor-run deterministic). Validate: `execute` / `judge` |
