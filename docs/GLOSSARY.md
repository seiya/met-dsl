...
- **deps.yaml**: The dependency declarations required by each `spec`. It defines `component_id` / `profile_id` (and, optionally, the R1 harness `infrastructure_id`) and `version constraint`.
