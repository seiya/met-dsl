...
- **Write every output artifact directly with the `Edit` / `Write` tool**, to a path enumerated in the `allowed_file_tool_paths` of `output_manifests/<agent_run_id>.json`. This is uniform across extensions: managed JSON (`lineage.json`, `*_meta.json`, `verdict.json`, `aggregate_verdict.json`, `summary.json`, `diagnostics.json`, `perf.json`, `quality_check.json`, the snapshot `*.json`, …), source code (`*.f90` / `*.c` / `Makefile`), and `.yaml` / `.yml` / `.md` are all written this way. Immediately after launch confirm both the `allowed_output_paths` and `allowed_file_tool_paths` lists. You run under a mandatory `bwrap` sandbox whose write scope is exactly your `write_roots`; a write that stays inside `write_roots` is authorized by filesystem-diff containment at terminalization — there is no separate "gate evidence" step to perform.
- The MCP-owned audit logs (`mcp_command_log.jsonl`) are written **only** by the `build-runtime` MCP server as a side effect of running its tools; they appear in `allowed_output_paths` but **not** in `allowed_file_tool_paths`, and you must never write them with a file tool. Everything else in `allowed_output_paths` is also in `allowed_file_tool_paths`.
- `guarded-apply-patch` is **deprecated** and not required for any write. Do not route managed JSON through it: a top-level file-pin write_root (e.g. `lineage.json`) only accepts an in-place overwrite, which the `Write` tool performs; `git apply` (which `guarded-apply-patch` uses) fails on a bind-mounted file. Just `Write` the file.
...
Every output artifact — managed JSON (`lineage.json`, `*_meta.json`, `verdict.json`, …), source code, `.yaml` / `.md` — is written **directly with the `Write` / `Edit` tool** to a path listed in `allowed_file_tool_paths`. You run inside a mandatory `bwrap` sandbox whose only writable paths are your `write_roots`; a `Write` to an `allowed_file_tool_paths` entry is authorized by filesystem-diff containment at terminalization (a change that lands inside `write_roots` is your own confined output). There is no patch step and no separate gate evidence to record.

**Procedure:**

1. To overwrite an existing file, optionally `Read` it first to compute the new content. The `Write` tool overwrites in place; it works whether or not the file already exists, including a top-level file-pin write_root (`lineage.json`) that an in-place overwrite is the *only* way to update under `bwrap`.
...
> **Deprecated:** `guarded-apply-patch` and `run-gate --gate apply_patch_writes` are no longer part of the write path and must not be used for artifact writes. `guarded-apply-patch` relies on `git apply` (unlink+create), which fails on a `bwrap` bind-mounted file-pin write_root; the `Write` tool's in-place overwrite is the correct mechanism. These commands are retained only for backward compatibility pending removal.
