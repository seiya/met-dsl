...
- The MCP-owned audit logs (`mcp_command_log.jsonl`) are written **only** by the `build-runtime` MCP server as a side effect of running its tools; they appear in `allowed_output_paths` but **not** in `allowed_file_tool_paths`, and you must never write them with a file tool. The pipeline `lineage.json` is likewise NOT yours — it sits at the pipeline root (outside your write_roots) and the conductor authors it host-side. Everything else in `allowed_output_paths` is also in `allowed_file_tool_paths`.
- `guarded-apply-patch` is **deprecated** and not required for any write. Write managed JSON directly with the `Write` / `Edit` tool to a path in `allowed_file_tool_paths`; `git apply` (which `guarded-apply-patch` uses) fails on a bind-mounted file, so do not use it.
