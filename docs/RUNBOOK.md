...
- The `--stage pre_judge` gate is run by the **conductor**, not the `Validate.judge` leaf (G3). The conductor runs a pre-spawn dependency-DAG readiness check (a `spec.ir.yaml.dependency.all_nodes` closure node not built+validated in its own pipeline fails the phase `fail_closed` before the judge is spawned) and, after the judge returns, `validate_pipeline_semantics --stage pre_judge` scoped to this run (`--pipeline-root`/`--run-id`, `--in-flight-agent-run-id` = the judge child, never `--allow-missing-orchestration` / `--allow-missing-llm-review`), recording the verdict in `judge_gate_meta.json`. A `pre_judge` `fail` is a non-physics integrity blocker the conductor terminalizes `fail_closed`. Operators do not invoke `pre_judge` for a judge by hand during a normal run.
...
- The conductor-run `--stage pre_judge` gate passes — recorded in `runs/<run_id>/<node_key_safe>/judge_gate_meta.json` as `status=pass` (the judge leaf no longer runs it; see §Validate).
