...
- **Reproduction record (`orchestration_meta.json#invocation`)**: every cold init records how the run was invoked — the raw command (`argv` / `command`) and the resolved/canonical params (`spec_ref` / `until_phase` / `llm` / `llm_command` / `mode` / `agent_model` / `with_deps`), plus the closure back-link for `--with-deps` nodes. It is captured once at the initial start and preserved across re-init / `--resume` (the same lifecycle as `repo_revision`), so it always reflects the invocation that produced the run.
