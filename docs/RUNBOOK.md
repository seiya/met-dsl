...
- `Validate.execute` 完了前に、`run-gate` で `validate_pipeline_semantics` を `--stage post_execute` 相当引数で実行する。`--pipeline-root` は繰り返し指定可能とし、`spec.ir.yaml.dependency.all_nodes` が複数 `node` を保持する試行では、`all_nodes` に対応する全 `pipeline_root` を `--pipeline-root` へ展開して実行する。`fail` 時は `Validate.execute` を `fail` とし、`Validate.judge` を開始してはならない。
