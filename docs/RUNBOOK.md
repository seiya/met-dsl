...
- **`generate-executor` is immutable across `--resume`** (`Z2`): the `generate-executor` (`legacy` / `pure`, `docs/GLOSSARY.md`) is recorded once in `orchestration_meta.json#invocation.generate_executor` at cold init and is authoritative on every resume — the **recovered value always wins**. A resume must not silently switch the execution + write-authority model: an implicit `--resume` (no flag, no env) recovers the recorded value; a resume-time `--generate-executor` that **conflicts** with the recorded value is **rejected** (`generate_executor_immutable_on_resume`); and the ambient `METDSL_GENERATE_EXECUTOR` env is **ignored** at resume (an unrelated env value cannot silently flip an in-flight run, and a stale env typo cannot block a resume). The executor value is validated only on cold init. Because a commit-crossing `--resume` is unsupported anyway, this only guarantees that a within-commit resume continues under the executor the run started with. The probed backend's CLI version (`preflight.json#agent_version` — `claude --version` on a `claude` run, `codex --version` on a `codex` run; `preflight.json#backend` says which) is likewise machine-local provenance, surfaced by `tools/audit_orchestration.py` (the pure-leaf A/B section) alongside the recorded executor.
