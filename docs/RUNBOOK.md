...
7. **`node` 単位 workflow 発行**: `orchestration agent` が各 `node_key` ごとに個別 `plan_id` と個別 `pipeline_id` を発行する。`substep` を持つ phase では `substep agent` を直接起動し、標準 `substep` を持たない phase では `step agent` を起動する。
