...
- The `--stage pre_judge` gate is run by the **conductor** as two deterministic substeps wrapping the `Validate.judge` leaf (G4), not by the judge leaf. The `pre_judge` substep (index 0) is a dependency-DAG readiness check (a `spec.ir.yaml.dependency.all_nodes` closure node not built+validated in its own pipeline authors `pre_judge_meta.json` `status=fail` and terminalizes `fail_closed` before execute/judge). The `post_judge` substep (index 3) runs `validate_pipeline_semantics --stage pre_judge` after the judge returns, scoped to this run (`--pipeline-root`/`--run-id`, with both the post_judge substep's own arid AND the judge arid as `--in-flight-agent-run-id`, never `--allow-missing-orchestration` / `--allow-missing-llm-review`), recording the verdict + a severity `disposition` in `post_judge_meta.json`. (Naming caution: the `post_judge` substep runs the validator stage literally named `pre_judge`.) A recoverable (judge-authored) violation warm-resumes the judge to re-author its artifact; an integrity/unknown violation, and any `pre_judge` `fail`, is a non-physics integrity blocker terminalized `fail_closed`. Operators do not invoke `pre_judge` by hand during a normal run.
