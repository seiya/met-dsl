...
- **Reading a `leaf_transport_error` fail_closed**: the reason has the form `leaf_transport_error: leaf_exit=<rc> (tag: <infra_tag>; <evidence>)`, followed by ` [attempts=<n>]` when the substep was launched more than once. **Route on the tag, not on the suffix.** The tag names the cause of the *last* attempt and says what to do (below); `[attempts=<n>]` only counts the launches, and a run may mix tags (a transport flake retried into a usage limit ends with `attempts=2` and a `llm_usage_limit` tag — the quota is what must be waited out, not a provider outage).
  ...
  - `llm_usage_limit`: wait for the quota to reset, then `--resume`. To have the conductor do this **automatically, in place**, re-run with the opt-in `--wait-usage-reset` flag (default OFF): when the dead leaf's terminal usage-limit line **on stderr** carries a resolvable reset — a machine `|<unix-epoch>` OR a TZ-anchored human reset (`resets 10:20pm (Asia/Tokyo)`, the form the real CLI emits) — the conductor sleeps to the reset (+120s margin) and re-launches the same substep instead of fail-closing, a same-run, substep-granular resume that avoids re-paying the earlier passed substeps a phase-granular `--resume` would re-run. A human reset **without a parenthesized IANA timezone** (`resets 6:10pm`) or without a time-of-day (`resets Monday`) is NOT resolved (the instant is never guessed from the host's local timezone), and a usage limit on the leaf's untrusted stdout declines the wait — both just `fail_closed` (and emit `leaf_usage_limit_wait_declined` with a reason). Bounded: one wait per substep, at most 6h (a longer reset falls back to `fail_closed`). The flag is **not** carried across `--resume` (a per-invocation preference recorded on `orchestration_meta.json#invocation` and refreshed to the effective value on resume) — re-pass `--wait-usage-reset` on the resume to keep it active. Canonical: `docs/ORCHESTRATION.md` "leaf transient retry".
