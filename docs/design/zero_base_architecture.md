...
- **Z2. Host-mediated `generate.generate` + `generate.verify`. (LANDED 2026-07-16 — canonical: `docs/workflow/phases/phase_02_generate.md` (Generate-executor), `docs/workflow/CODEGEN_BUNDLE_CONTRACT.md`, `tools/pure_leaf.py`; record: `deterministic_followups.md` "Z2". The `pure` `generate-executor` runs the migrated `(generate, generate)` / `(generate, verify)` leaves as pure-function leaves on a `claude` backend `M3c` node; `legacy` remains the default until the billed A/B adoption commit flips it. A non-`M3c` / `codex` node stays `legacy` even under `--generate-executor pure`.)** The host assembles the optimization-unit IRs, dependency facts, exemplar, target profile, and harness capability ABI. `generate.generate` returns one complete `CodegenBundle`; it does not return algorithm-step fragments for conductor concatenation. The host writes the declared files, deterministically assembles boundary glue and the build graph, and runs the existing lint/syntax/static gates. `generate.verify` reviews the bundle and assembled-source contract in an independent conversation. Closed-context execution is the default; bounded named-artifact reads are permitted only when the context-size policy requires them. This is the dominant-cost leaf and is expected to capture most of the prefix-caching benefit while retaining internal code-organization and hardware-optimization freedom.
