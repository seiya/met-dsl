...
| MCP `build-runtime` spawned in-sandbox | the generate/build/validate leaves recorded `run_linter` / `compile_project` / `run_program` evidence (`command_log.jsonl` present, `ok:true`) |
