...
- **Deterministic in-process execution (opt-in).** The non-LLM steps `Build` and `Validate.execute` may be run by the conductor **in-process** instead of as a leaf, gated by `METDSL_CONDUCTOR_DETERMINISTIC_BUILD` / `METDSL_CONDUCTOR_DETERMINISTIC_EXECUTE` (default off). This breaks the otherwise-uniform "every `step` / `substep` is a leaf agent" model for those two steps only. To keep the integrity layer (executor-role / launch-marker / capability checks) unchanged, the conductor **plays the child role**: it issues the same `record-launch` → `record-child-return` → `finalize-child` bookkeeping so the executor stays a normal `step` / `substep` `agent_run_id`. `Validate.judge` is never run this way (its independent LLM semantic check is essential). See [`docs/workflow/phases/phase_03_build.md`](workflow/phases/phase_03_build.md) and [`phase_04_validate.md`](workflow/phases/phase_04_validate.md).
