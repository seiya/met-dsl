...
- launch request templates (rendered by the conductor's `record-launch`): [`skills/workflow-orchestration/references/launch_prompts.md`](../skills/workflow-orchestration/references/launch_prompts.md)
...
- **verify-family meta re-author (still required for a different reason):** a `compile.verify` / `generate.verify` substep must produce its declared meta output (`ir_meta.json` / `source_meta.json`) to terminate `pass` — an inspect-only verify that writes nothing cannot `pass` (it has no `output_refs`). When the inspection finds nothing to change, re-author the meta anyway (refresh an idempotent field such as `verify_attempts`) with `Edit` / `Write`. The verify launch prompt MUST instruct the meta re-author as mandatory (not optional) — see the verify-family line of the `substep agent` template in [skills/workflow-orchestration/references/launch_prompts.md](../skills/workflow-orchestration/references/launch_prompts.md).
...
- The launch prompt to a child `agent` is based on the corresponding template in `skills/workflow-orchestration/references/launch_prompts.md`, with the in-template placeholders substituted by the actual values of the target `agent_run`. `record-launch` performs this rendering; the conductor supplies the request parameters rather than the rendered prompt.
