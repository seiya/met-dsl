...
- **`agent_session_id` is synthetic, not a live session.** The conductor reuses the child's `agent_run_id` (UUID) as its `agent_session_id` / `context_id`. These are **bookkeeping identifiers only**: there is no `~/.claude/projects/.../<agent_session_id>.jsonl` transcript addressable for a child leaf, and the child does not emit a separate session file or sidechain transcript. The canonical record of a child's work is `workspace/orchestrations/<orchestration_id>/agents/<agent_run_id>/dialogs/` (the launch prompt/request, and the final reply/summary). Do not look for a child under `~/.claude`.
