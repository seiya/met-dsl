...
- The child-`agent` launch prompt must be rendered from the corresponding template in `skills/workflow-orchestration/references/launch_prompts.md`; a free-form prompt that does not use the template is forbidden. The orchestration agent supplies the launch-request parameters and does not author the prompt itself: the rendering is done by `record-launch` (the canonical render path). The per-backend mechanics differ — on the Claude Code backend `record-launch` runs before the Agent tool and returns the rendered `launch_prompt_text` (the orchestration must not read the template), while on Codex/Cursor the orchestration renders before `spawn_agent`. See `skills/workflow-orchestration/SKILL.md` Operations Rule 7 / Rule 12 and the project `CLAUDE.md` for the backend-specific procedure.
...
- The launch prompt to a child `agent` is based on the corresponding template in `skills/workflow-orchestration/references/launch_prompts.md`, with the in-template placeholders substituted by the actual values of the target `agent_run`. `record-launch` performs this rendering (see the backend-specific note above); the orchestration agent supplies the request parameters rather than the rendered prompt.
