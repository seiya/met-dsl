...
- Per-`node workflow` phase execution, and the execution of in-phase `substep` (`generate` / `lint` / `verify` / `execute` / `judge`)
...
| Generate | has substeps | `generate` / `lint` (conductor-run deterministic) / `verify` |
| Build | single step | - |
