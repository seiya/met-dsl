...
### repair / retry
- The conductor must evaluate the child `agent`'s returned result and judge `issue_severity` (`minor` / `major` / `critical`).
- The conductor must judge whether re-submission is needed based on `issue_severity` and the scope of contract deviation, and when re-submission is needed, must choose `repair_strategy` (`reuse` / `restart`).
...
- **Warm-resume reuse (claude backend).** For the claude backend, a `reuse` repair whose target session transcript still exists is warm-`--resume`d (forked) so the repair leaf inherits the producer's context instead of cold-starting; gated by `METDSL_CONDUCTOR_REUSE_RESUME`, falling back to a cold launch if the transcript was GC'd. For the specific `Generate.lint`/`Generate.static` finding reopen, when warm resume fires the conductor may send a **slim findings-only repair turn** (the resumed leaf already holds the SKILL/contract docs) — gated by `METDSL_CONDUCTOR_REUSE_SLIM_PROMPT`. Canonical: [docs/design/deterministic_followups.md](design/deterministic_followups.md) "G1-slim".
