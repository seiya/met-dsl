...
- **Deterministic in-process execution.** The non-LLM steps `Build` and `Validate.execute` are **always** run by the conductor **in-process** (no `claude -p` leaf, no `SKILL.md`) — it calls the build-runtime MCP handlers (`compile_project` / `run_program` / `run_quality_checks`) directly. The in-process bodies implement the in-source **Make** layout only (`build_system=make`; OBJDIR/BINDIR overrides, `make_test`); a non-Make `build_system` (cmake/meson/ninja) is rejected with a clear error rather than misplaced. A content failure (compile/link error, post_build/post_execute violation, quality-check mismatch, runtime error) returns cleanly and routes via the Build/Validate decision tables (→ Generate etc.), exactly like the leaf era; only a genuine conductor/MCP infra failure fail-closes as a transport error. They are the only steps that are not leaf agents. The conductor still issues the `record-launch` → `record-child-return` → `finalize-child` bookkeeping so the executor is a normal `step` / `substep` `agent_run_id` with a capability and phase_state, but their launch prompt is a **minimal deterministic stub** (sentinel `Conductor-executed deterministic step` + core ids; no skill section), and the validator applies a reduced launch-prompt-marker set to it. `Validate.judge` is always a leaf (its independent LLM semantic check is essential). See [`docs/workflow/phases/phase_03_build.md`](workflow/phases/phase_03_build.md) and [`phase_04_validate.md`](workflow/phases/phase_04_validate.md).
