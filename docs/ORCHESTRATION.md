...
- **verify-family no-write trigger (canonical reuse case):** a `compile.verify` / `generate.verify` substep that reaches a `pass` judgment but emits **no `apply_patch_writes` gate evidence** (it inspected only and re-authored no meta) is rejected by `record-agent-run` (`pass status for substep requires apply_patch_writes gate evidence`). The conductor must record that run `fail` (`issue_severity=minor`) and re-submit with `repair_strategy=reuse` and a fresh `agent_run_id`; the reuse re-authors the meta (`ir_meta.json` / `source_meta.json`) through `guarded-apply-patch` and satisfies the gate. The verify launch prompt MUST instruct the meta re-author as mandatory (not optional) to avoid this round-trip — see the verify-family line of the `substep agent` template in [skills/workflow-orchestration/references/launch_prompts.md](../skills/workflow-orchestration/references/launch_prompts.md).
...
> **Audience note:** this section is the orchestrator/conductor-facing specification of `guarded-apply-patch`. The **child-facing** canonical procedure (what a step/substep agent follows) is [docs/AGENT_CONTRACT.md](AGENT_CONTRACT.md) §"`.json` / `.txt` artifact write" — agents read AGENT_CONTRACT, not this file (`docs/ORCHESTRATION.md` is not in any child's `must_read`).
