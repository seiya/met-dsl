...
- Per-`node workflow` phase execution, and the execution of in-phase `substep` (`generate` / `lint` / `static` / `verify` / `pre_judge` / `execute` / `judge` / `post_judge`)
...
| Generate | has substeps | `generate` / `lint` (conductor-run deterministic) / `static` (conductor-run deterministic) / `verify` |
| Build | single step | - |
...
- The changes to phase artifacts permitted to a child `agent` must be limited to under the `write_root` permitted by the capability token.
  - `Compile.generate` / `Compile.static` / `Compile.verify`: `workspace/ir/<node_key_safe>/<ir_id>/` (`Compile.static` writes `compile_static_meta.json`)
  ...
  - `Validate.pre_judge` / `Validate.execute` / `Validate.judge` / `Validate.post_judge`: `workspace/pipelines/<node_key_safe>/<pipeline_id>/runs/<run_id>/<node_key_safe>/` (the two gate substeps author `pre_judge_meta.json` / `post_judge_meta.json` there)
...
- `record-agent-run` authorizes a step/substep child's writes by `write_roots`-containment of the actually-changed paths (the diff against the baseline): a change that landed inside the capability token's `write_roots` is the child's own confined output and is authorized; a change outside `write_roots` is rejected as an `unauthorized write`. (Under mandatory `bwrap` a write outside `write_roots` is impossible by construction.) The canonical MCP audit logs are handled by their own placement check. The conductor's host write of the `generate.lint` certificate `<pipeline_root>/lint_evidence/<source_id>.json` — made in-process during the lint substep window and deliberately at the pipeline root (non-forgeable, outside the substep's `source/` write_root, like `lineage.json`) — is exempted, scoped to step==`generate` ∧ substep==`lint` so the sandboxed `Generate.generate` leaf is never exempted. The exemption is bound to that EXACT certificate filename (the `<source_id>` from the host-authored launch request), not the whole `lint_evidence/` directory, so a stray/unexpected sibling there is still flagged. No `apply_patch_writes` gate record is consulted for step/substep.
- **runtime placeholder restoration: removed (P2-7).** The `created_file_pin_stubs` 0-byte placeholder restoration `record-agent-run` previously performed is gone, together with the file-pin stub system. It is unnecessary now: the core workflow declares no file-pin write_root (`lineage.json` is authored host-side by the conductor; the pipeline root is non-writable to the sandboxed leaf), so there is no runtime-owned placeholder a leaf could collaterally delete.
...
- For a `step` that has `substep`, the `agent_run_id` of all `substep` of that `step` recorded in `agent_runs.jsonl` is included in the `substep_agent_run_ids` of some `step_result.json`. A `substep` / `step` recorded in `reopen/superseded_runs.json` (tombstoned by a `reopen-phase` cross-phase retry; its `step_result.json` was archived to `step_result.superseded.<seq>.json`) is exempt from this vouch requirement. A reopened `(node, phase)` (every `(node_key, step)` carried by a superseded run) must nonetheless regain at least one fresh non-superseded terminal run before `pass`; otherwise `_validate_orchestration_completion_for_pass` rejects the `pass`, so a `reopen-phase` that archived the only evidence cannot be marked `pass` until the replacement attempt runs.
