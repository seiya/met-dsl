...
- workflow 共通の不変規範（過去 artifact 参照禁止、`dummy` 禁止、検証契約導出、`workspace/` ルート制約、`quality check` 判定軸）は `WORKFLOW.md` を canonical source とする。
...
6. workflow 共通規範の変更は `WORKFLOW.md`、階層実行契約の変更は `ORCHESTRATION.md`、phase 手順の変更は対応 `SKILL.md` へ記述する。
