...
- The runner program-output contract (`diagnostics.json` / `perf.json` / `raw/` composition + the Fortran JSON-descriptor rules) is canonical in `docs/workflow/RUNNER_OUTPUT_CONTRACT.md`, and is the leaf must-read for `Generate.generate` / `Generate.verify` / `Validate.judge`. It supersedes the runner-output prose previously duplicated in `phase_02_generate.md` / `phase_04_validate.md` / `PERFORMANCE_DIAGNOSTICS.md`; new runner-output rules go there, and those docs reference it.
- **Leaf must-read minimization (canonical: `docs/design/leaf_must_read_restructure.md`).** `WORKFLOW_CORE.md`, `ORCHESTRATION.md`, and the per-phase `phase_*.md` are **not** force-read by LLM leaves (they are orchestration/operator canonical and stay readable under `docs/`); the leaf-actionable subset of `WORKFLOW_CORE.md` lives in `AGENT_CONTRACT.md`. The phase doc is force-read only for `Compile` (`phase_01`'s IR schema). When adding a leaf-actionable rule, prefer `AGENT_CONTRACT.md` (every leaf), the per-substep `SKILL.md` (one substep), or `RUNNER_OUTPUT_CONTRACT.md` (runner output) so it reaches the leaf without re-bloating the must-read set.
