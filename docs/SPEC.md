...
## 最終ゴール
`Controlled Spec` と `tests` を正本として、`CPU` / `GPU` などのハードウェア上で各 `spec` が定義する計算課題を実装するサブルーチン群（`model`）と、入出力・実行・判定連携を担う `runner` を運用可能な品質で生成する。
...
## 不変原則
1. ドキュメントを正本とする。`Controlled Spec` と `tests` は、ドメイン研究者が単独で解釈でき、判定入力へ決定的に変換できなければならない。
2. 妥当性保証は出口で行う。`LLM` の非再現性を前提に受け入れ、最終品質は実行結果に基づく判定で保証する。
3. 物理計算を担う `model` と、入出力・判定連携を担う `runner` を分離する。

## 不正防止原則（全体仕様）
1. `tests` 合格または workflow 進行を目的とした `dummy` 出力の生成を禁止する。
2. 工程入力が不足する場合は当該工程を `fail` で停止し、推測補完やプレースホルダ補完を禁止する。
3. 工程失敗時に下流工程の開始条件を満たす目的で成果物を人工生成してはならない。
...
- `debug_mode` の既定値は `false` とする。`debug_mode=true` の場合のみ失敗試行成果物の保存を許可し、`retained_failed_attempts` と保存先を記録する。
...
3. `domain` と `family` の定義は `GLOSSARY.md` の「`spec` 分類語彙」に一致させる。
...
14. 正式版成果物は `spec` 配下に配置してはならない。保存先は `releases/<spec_kind>/<domain>/<family>/<spec_id>/<target_architecture>/<toolchain_language>/<release_id>/` を必須とする。
15. 粒度判定は次を必須基準とする。
...
- `workspace/` は試行成果物の作業領域とし、正式版成果物の正本に使用してはならない。
...
- 互換移行期間では旧配置 `spec/<domain>/<family>/<spec_id>/...` を `problem spec` とみなし、`spec_kind=problem` で台帳登録する。
...
- `problem` / `component` / `profile` の責務境界が維持され、依存宣言が台帳と整合している。
- 物理妥当性判定と性能評価に必要な成果物契約が workflow で再現可能である。
