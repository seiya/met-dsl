...
- `substep` refers to a lower execution unit decomposed from a `step`.
  ...
  - `Generate` has the 5 substeps `generate`, `lint`, `syntax`, `static`, and `verify` — `lint`, `syntax` and `static` are deterministic conductor-run substeps (no leaf) between `generate` and `verify` (`syntax` runs the MCP `run_syntax_check` compiler front-end gate, gfortran `-fsyntax-only`; `static` runs `validate_workspace_root` then `validate_pipeline_semantics --stage post_generate`); a `lint`, `syntax` or `static` finding in the node's own source warm-resumes `generate` (a `syntax` finding the staged dependency closure causes instead fails closed — the leaf cannot write it), so the LLM `verify` is reached only on a deterministically-clean source.
...
- output: `source/<source_id>/src/`, `source_meta.json`, `lint_meta.json` / `syntax_meta.json` (the conductor-authored `Generate.lint` / `Generate.syntax` deliverables; see next bullets)
- The `Generate.generate` / `Generate.verify` leaves run under the `pure` `generate-executor` — the only executor since `M-F` (legacy generate execution removed; `docs/GLOSSARY.md`). The value is recorded (always `pure`) in `orchestration_meta.json#invocation.generate_executor`, and a resume of a non-`pure` record is rejected fail-closed (`generate_executor_legacy_removed`). On a `claude` backend `M3c` node `Generate.generate` is a `pure-function leaf` returning one `CodegenBundle` that the host validates (`validate_bundle`) and writes, and `Generate.verify` is a `pure-function leaf` returning a verify verdict the host projects onto `source_meta.json`; the source directory additionally holds `codegen_bundle.json`, `bundle_meta.json`, `verdict_meta.json`. A residual node the pure producer cannot express (a `codex` backend, or a node whose runner/Makefile are not host-rendered) runs the shared **agentic** leaves, which write the source and re-author `source_meta.json` directly. The deterministic `Generate.lint` / `Generate.syntax` / `Generate.static` substeps and the authoring rules below are identical either way. Canonical: `phases/phase_02_generate.md`.
...
- After `Generate.lint` and before `Generate.static`, the conductor's deterministic `Generate.syntax` substep runs the MCP `run_syntax_check` compiler front-end gate for `toolchain.language=fortran` (mandatory gfortran `-fsyntax-only -std=<toolchain.standard> -Werror=unused-dummy-argument -Werror=unused-variable` over the node + staged dependency-closure sources; optional `METDSL_SYNTAX_COMPILERS` target-compiler stages are recorded `skipped` when their binary is not installed) and writes both `source/<source_id>/syntax_meta.json` and the host-authored syntax evidence (`<pipeline_root>/syntax_evidence/<source_id>.json`, per-stage `compiler` / `status` / `command_id` / `command_log_ref`); `post_generate` certifies it (the gfortran stage must have passed). A compiler finding in the node's own source warm-resumes `generate`; a failing stage the deterministic attribution runs trace to an unviable invocation (a `toolchain.standard` the compiler driver rejects) or to the staged dependency closure is a transport `fail_closed` naming what to fix, since the leaf authors neither; non-fortran languages pass through. The leaf does not run `run_syntax_check`.
- After `Generate.syntax` and before `Generate.verify`, the conductor's deterministic `Generate.static` substep runs `validate_workspace_root.py` then `validate_pipeline_semantics --stage post_generate` (writing `source/<source_id>/static_meta.json`) — the purely-static gates the verify leaf used to own. A violation warm-resumes `generate`; otherwise `Generate.verify` runs as a pure LLM semantic pass that launches no validator gate. The leaf does not run these validators.
