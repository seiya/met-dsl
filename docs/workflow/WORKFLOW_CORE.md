...
- The `Generate.generate` / `Generate.verify` leaves run under one of two `generate-executor` selections (`docs/GLOSSARY.md`), recorded in `orchestration_meta.json#invocation.generate_executor`. Under `legacy` the agentic leaves write the source and re-author `source_meta.json` directly. Under `pure` (`Z2`, a `claude` backend `M3c` node only) `Generate.generate` is a `pure-function leaf` returning one `CodegenBundle` that the host validates (`validate_bundle`) and writes, and `Generate.verify` is a `pure-function leaf` returning a verify verdict the host projects onto `source_meta.json`; the source directory additionally holds `codegen_bundle.json`, `bundle_meta.json`, `verdict_meta.json`. The deterministic `Generate.lint` / `Generate.syntax` / `Generate.static` substeps and the authoring rules below are identical under both. Canonical: `phases/phase_02_generate.md`.
