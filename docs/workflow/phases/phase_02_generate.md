...
- `impl_defaults.toolchain.build_system=make` かつ `impl_defaults.toolchain.language=fortran` の場合、オブジェクト規則のターゲットは **literal 名**（例: `foo.o:`、out-of-source なら `$(OBJDIR)/foo.o:`）で記述する。`$(OBJ)` のみのような変数展開だけをターゲットとする規則行は依存欠落と判定される。`$(OBJDIR)/` 等の directory prefix は静的解析が basename へ正規化するため許容される（`.o`/`.mod` の literal basename が残ること）。
