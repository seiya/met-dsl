...
- `impl_defaults.toolchain.build_system=make` かつ `impl_defaults.toolchain.language=fortran` の場合、オブジェクト規則のターゲットは **literal 名**（例: `foo.o:`）で記述する。`$(OBJ)` のみのような変数展開だけをターゲットとする規則行は依存欠落と判定される。
...
- `impl_defaults.toolchain.build_system=make` かつ `impl_defaults.toolchain.language=fortran` / `c` / `cpp` / `mixed` 系では、`source/<source_id>/src/Makefile` に `test` または `check` target を必須定義する。欠落時は `Generate fail` とする。
