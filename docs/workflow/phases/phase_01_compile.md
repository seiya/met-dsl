...
public_api:
  ...
  module_parameters:                      # copy each §5.1 module_parameters entry, value included (V8)
    - {name: dp, base: integer, value: real64}          # {name, value} (base: integer optional); gate
...
- **`public_api.module_parameters`** (same gate, `_validate_ir_module_parameters_against_section51`): pinned == the §5.1 block's `module_parameters` (each `{name, base?, value}`) by name AND value (values compared normalized, so `64` == `"64"` and `real64` == `REAL64`; `base` is not compared). Like the signatures, Compile.generate transcribes these because `Generate.generate` is walled off from `controlled_spec.md` — the IR is the leaf's only carrier of the values (`dp` / `case_id_len`). A missing key (when §5.1 declares any), a non-list key, a malformed/duplicate entry, or an omitted/extra/drifted value is a `Compile fail`; the declarations are value-pinned against the generated source at `Generate.static`.
