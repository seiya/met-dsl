...
- `runner`（`run_program` が実行する binary）は canonical run dir へ `diagnostics.json` / `perf.json` を**直書きしてはならない**。canonical `.json` は `guarded-apply-patch` の gate evidence を必須とし、binary 直書きは `unauthorized_write_violation` → `fail_closed` を招く（`mcp_command_log.jsonl` のような MCP-owned audit log とは扱いが異なる）。binary の出力は `allowed_tmp_root`（`workspace/tmp/<exec_agent_run_id>/`）へ落とし、`Validate.execute` agent が `run/` tmp tree（`workspace/tmp/<exec_agent_run_id>/run/`、= `run_program` 実行の出力）の `.json` を読んで canonical `diagnostics.json` / `perf.json` を `guarded-apply-patch`（create-form）で再 author する。runner のプログラム出力（`diagnostics.json` / `perf.json` / `raw/metrics_basis.json` / `raw/state_snapshots/*`）は必ず `run/` から promote し、`qc_run/`（`run_quality_checks` / make test 再実行の出力）から promote してはならない（promote すると `Validate.judge` が provenance 不一致の証跡を消費する）。`qc_run/` は quality-check 比較（`quality_check.json`）のためにのみ参照する。`trial_meta.json` と `quality_check.json` は runner 出力ではなく agent が author するメタデータ（runner は直接出力してはならない）であり、tmp からの promote ではなく `guarded-apply-patch` で生成する。`raw/` 配下の `.json`（`metrics_basis.json` / `state_snapshots/*.json` 等）も canonical `.json` のため `guarded-apply-patch` で再 author する。`Write` tool で書くのは `raw/` 配下の非 `.json` ファイル・`stdout.log`・`stderr.log` に限る。
