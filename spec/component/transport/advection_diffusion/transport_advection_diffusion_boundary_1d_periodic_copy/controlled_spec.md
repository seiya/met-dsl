...
## 0. メタ情報
- `spec_id`: `transport_advection_diffusion_boundary_1d_periodic_copy`
- `spec_version`: `0.1.0`
...
## 3. 演算定義
公開 `operation` は `transport_advection_diffusion_boundary_1d_periodic_copy__apply` とする。`ng=1` のとき
$$
...
## 5. 公開 API と互換性
公開 `operation_id` は `transport_advection_diffusion_boundary_1d_periodic_copy__apply` のみとする。`major` 互換破壊時は `spec_id` を分離する。
