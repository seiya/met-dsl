# Tests: 2 次元 shallow water 既定プロファイル
...
## 1. テスト目的
本スイートは、`shallow_water` 問題に対する既定プロファイル選択規則と互換性ガードを検証する。

## 2. 入力既定化規則
- 正常系は `problem.family=shallow_water`、`dimension=2d`、`boundary=periodic` を入力とする。
...
- 異常系は互換範囲外版として `dynamics_shallow_water_time_update_2d_ssprk2@1.0.0` を使用する。

## 3. 実行制御規則
本スイートは `profile` 選択ロジックの判定のみを対象とする。時間積分の実行制御は `N/A` とする。理由は「本スイートがプロファイル解決のみを検証対象とするため」とする。
...
- `test_id`: `l0_select_default_profile_pass`
  ...
  - `判定`: `profile_id=dynamics_shallow_water_profile_2d_rusanov_p0_ssprk2` が選択され、`checks.profile_selected=true` を満たす。
