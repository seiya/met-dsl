...
- The metric case (`l0_metric_leaf_pass`) supplies exactly two fixed sentinel `h_metric` records and no other sentinel payload (it carries the same `grid` / `time` / `boundary` / profile-selection inputs as every other case). The two records, whose field values are the case's input data, are:
...
- The per-case `metrics` object is a **fold over that case's supplied `h_metric` array**: `harness_fortran_cpu__write_diagnostics` iterates the array and writes one leaf per record (a numeric record as `"<name>": <value>`; an `is_na` record as `"<name>": null` plus the sibling `"<name>_reason_na": "<reason_na>"`). The object is decided by the supplied records alone — a `metrics` body that is a literal, or that is selected by `case_id`, is forbidden even when it reproduces the expected keys for this suite, because the consuming physics nodes supply different records.
