...
"""
    ...
    def test_detects_runner_diagnostics_hardcoded_integers(self) -> None:
        # A fabricated diagnostics block that bakes bare INTEGER JSON values (0/1 for
        # each physics check) must still be flagged: the constant-heavy signal counts
        # int and float literals embedded in the emitted payload alike.
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            _seed_shape_expr_schema_into(repo_root)
            model_text = """module shallow_water2d_model
...
call shallow_water2d__step(cfl_max, h_min, mass_drift_rel)
open(newunit=u, file='diagnostics.json', status='replace', action='write')
write(u,'(a)') '{"cfl":0,"positivity":1,"mass":0,"momentum":1,"analytic":0,"convergence":1}'
close(u)
end program shallow_water2d_runner
"""
            _create_minimal_execution_tree(
            ...
            self.assertTrue(
                any("diagnostics block does not reference model call arguments" in v for v in violations),
                msg=f"integer-hardcoded fabrication was not flagged; violations={violations}",
            )

    def test_detects_runner_diagnostics_formatted_write_arg_constants(self) -> None:
        # A fabricated block that emits constants as formatted WRITE ARGUMENTS (outside
        # any single payload string) must still be flagged: numeric literals in a
        # `write` data list are emitted output, even though the format descriptor's own
        # digits (inside the control-list parens) are not counted. Kind-suffixed
        # literals (`0.72_real64`, `100_8`) count too — the trailing `_kind` must not
        # let a constant slip past the standalone-numeric boundary.
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            _seed_shape_expr_schema_into(repo_root)
            model_text = """module shallow_water2d_model
...
call shallow_water2d__step(cfl_max, h_min, mass_drift_rel)
open(newunit=u, file='diagnostics.json', status='replace', action='write')
write(u,'(a,f8.3)') '"cfl":', 0.72_real64
write(u,'(a,f8.3)') '"hmin":', 0.91
write(u,'(a,es12.3)') '"mass_drift":', 1.0e-12_real64
write(u,'(a,es12.3)') '"momx_drift":', 1.0e-12
write(u,'(a,es12.3)') '"momy_drift":', 1.0e-12
write(u,'(a,i0)') '"steps":', 100_8
close(u)
end program shallow_water2d_runner
"""
            _create_minimal_execution_tree(
            ...
            self.assertTrue(
                any("diagnostics block does not reference model call arguments" in v for v in violations),
                msg=f"formatted-write-arg fabrication was not flagged; violations={violations}",
            )

    def test_allows_struct_delegated_runner_diagnostics(self) -> None:
        # Regression for the constant-heavy false positive: a legit runner computes
        # results via the model, stores them in a results struct, and writes
        # diagnostics.json by delegating to helper subroutines whose only literals are
        # integer check ordinals. Infrastructure calls (build_case_table) precede the
        # model call, so the gate must key on the model routine (not the first call)
        # and must count only floating-point literals.
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            _seed_shape_expr_schema_into(repo_root)
            model_text = """module shallow_water2d_model
...
implicit none
contains
subroutine sw2d_step(h, hu, hv, cfl_max, h_min, mass_drift_rel)
  real(8), intent(inout) :: h, hu, hv
  real(8), intent(out) :: cfl_max, h_min, mass_drift_rel
  call dynamics_shallow_water_flux_2d_rusanov_p0__compute_flux(h, hu, hv)
  cfl_max = h + hu
  h_min = min(h, hv)
  mass_drift_rel = abs(h - hu)
end subroutine sw2d_step
end module shallow_water2d_model
"""
            runner_text = """program shallow_water2d_runner
use shallow_water2d_model, only: sw2d_step
implicit none
type :: case_result
  real(8) :: cfl_max = 0.0d0
  real(8) :: h_min = 0.0d0
  real(8) :: mass_drift_rel = 0.0d0
end type case_result
type(case_result) :: res
real(8) :: h, hu, hv
integer :: u, table
call build_case_table(table)
h = 1.0d0
hu = 0.0d0
hv = 0.0d0
call sw2d_step(h, hu, hv, res%cfl_max, res%h_min, res%mass_drift_rel)
open(newunit=u, file='diagnostics.json', status='replace', action='write')
write(u,'(a)') '{'
call write_suite_check(u, "cfl", res, 1)
call write_suite_check(u, "positivity", res, 2)
call write_suite_check(u, "mass_conservation", res, 3)
call write_suite_check(u, "momentum_conservation", res, 4)
call write_suite_check(u, "analytic_comparison", res, 5)
write(u,'(a)') '}'
close(u)
contains
subroutine build_case_table(t)
  integer, intent(out) :: t
  t = 0
end subroutine build_case_table
subroutine write_suite_check(unit, name, r, idx)
  integer, intent(in) :: unit, idx
  character(len=*), intent(in) :: name
  type(case_result), intent(in) :: r
  write(unit,'(a)') '  "'//trim(name)//'": {'
end subroutine write_suite_check
end program shallow_water2d_runner
"""
            _create_minimal_execution_tree(
                repo_root,
            ...
            self.assertFalse(
                any("diagnostics block does not reference model call arguments" in v for v in violations),
                msg=f"unexpected constant-heavy false positive; violations={violations}",
            )
