...
class OpsAndResolutionTest(unittest.TestCase):
    def test_ops(self) -> None:
        diag = {"m": 0.5, "s": "pass", "b": True, "lst": ["cfl", "input_guard"]}
        ...
        self.assertEqual(one("m", "le", 1.0), "pass")
        self.assertEqual(one("m", "ge", 1.0), "fail")
        self.assertEqual(one("m", "lt", 0.5), "fail")
        self.assertEqual(one("m", "gt", 0.4), "pass")
        self.assertEqual(one("s", "eq", "pass"), "pass")
        self.assertEqual(one("s", "ne", "fail"), "pass")
        self.assertEqual(one("b", "eq", True), "pass")
        self.assertEqual(one("lst", "includes", "cfl"), "pass")
        self.assertEqual(one("lst", "includes", "nope"), "fail")
    ...
    def test_includes_requires_list_not_string(self) -> None:
        ...
        diag = {"s": "passed"}
        pred = {"test_id": "t", "expected_outcome": "pass", "target_cases": [],
                "pass_when": {"all": [{"ref": "s", "op": "includes", "value": "pass"}]}}
        self.assertEqual(evaluate_predicate(pred, diag)[0], "fail")
    ...
    def test_per_case_empty_target_cases_is_non_satisfying(self) -> None:
        ...
        pred = {"test_id": "t", "expected_outcome": "pass", "target_cases": [],
                "pass_when": {"all": [{"ref": "metrics.x", "op": "le", "value": 1.0,
                                       "per_case": True}]}}
        status, kind, _ = evaluate_predicate(pred, {"cases": [{"case_id": "c", "metrics": {"x": 999}}]})
        self.assertEqual((status, kind), ("fail", "structural"))
    ...
    def test_includes_bool_number_disjoint(self) -> None:
        ...
        diag = {"lst": [True]}
        pred = {"test_id": "t", "expected_outcome": "pass", "target_cases": [],
                "pass_when": {"all": [{"ref": "lst", "op": "includes", "value": 1}]}}
        self.assertEqual(evaluate_predicate(pred, diag)[0], "fail")
...
class CaseResolutionTest(unittest.TestCase):
    ...
    def test_array_cases(self) -> None:
        diag = {"cases": [{"case_id": "n032", "metrics": {"mass_drift_rel": 1e-13}}]}
    ...
    def test_per_case_container_flat_entries(self) -> None:
        # per_case entries may be flat (no nested verdict), as demo_dep_top emits.
        diag = {"per_case": {"c": {"overall": "pass", "failed_checks": []}}}
        pred = {"test_id": "t", "expected_outcome": "pass", "target_cases": ["c"],
                "pass_when": {"all": [{"ref": "overall", "op": "eq", "value": "pass",
                                       "per_case": True}]}}
        self.assertEqual(evaluate_predicate(pred, diag)[0], "pass")
    ...
    def test_per_case_threshold_map(self) -> None:
        diag = {"cases": [{"case_id": "n032", "errors": {"analytic_h": {"l2_rel_tend": 0.21}}},
                          {"case_id": "n064", "errors": {"analytic_h": {"l2_rel_tend": 0.11}}}]}
        ...
        diag["cases"][1]["errors"]["analytic_h"]["l2_rel_tend"] = 0.13
        self.assertEqual(evaluate_predicate(pred, diag)[0], "fail")
...
class TwelveSpecExpressibilityTest(unittest.TestCase):
    ...
    def test_problem_nx_threshold_convergence_and_na(self) -> None:
        ...
        diag = {"cases": [
            {"case_id": "swe2d_ref_n032", "cfl": {"max": 0.45},
             "metrics": {"mass_drift_rel": 1e-13},
             "errors": {"analytic_h": {"l2_rel_tend": 0.20}}},
            {"case_id": "swe2d_ref_n064", "cfl": {"max": 0.45},
             "metrics": {"mass_drift_rel": 1e-13},
             "errors": {"analytic_h": {"l2_rel_tend": 0.11}}},
            {"case_id": "swe2d_ref_n128", "cfl": {"max": 0.45},
             "metrics": {"mass_drift_rel": 1e-13},
             "errors": {"analytic_h": {"l2_rel_tend": 0.06}}}],
            ...
            "convergence": {"n32_to_n64": {"analytic_h_order": 0.86},
                            "n64_to_n128": {"analytic_h_order": 0.87}}}
