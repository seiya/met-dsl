...
- Before `Validate.judge` starts and before it completes, run `python3 tools/validate_pipeline_semantics.py --stage pre_judge --orchestration-id <orchestration_id> --in-flight-agent-run-id <own agent_run_id>`, and on `fail` it is a `Validate.judge fail`. `--allow-missing-orchestration` and `--allow-missing-llm-review` must not be specified.
  - **Always attach `--orchestration-id <orchestration_id>` (`<...>` is the literal substitution of your run's `orchestration_id`).** The conductor runs one orchestration per node, so `workspace/orchestrations/` accumulates this run, its dependency runs, and debris from prior/crashed unrelated runs. `--orchestration-id` scopes the cross-orchestration integrity scan (dangling `agent_graph` edge / role-parent checks) to this single orchestration, so an unrelated crashed orchestration's debris cannot fail this healthy run. Dependencies are validated by their own `pre_judge` plus the launch-check readiness gate, so they are not re-policed here.
