...
- `impl_defaults.toolchain.build_system=make` かつ `impl_defaults.toolchain.language=fortran` の場合、`src/Makefile` は `use` 依存に対応したオブジェクト依存関係を明示し、依存 `module` の `.mod` または依存 `.o` を各オブジェクトターゲットの前提条件へ必須記述する。オブジェクト規則は **literal** ターゲット名（例: `foo.o:` または out-of-source なら `$(OBJDIR)/foo.o:`）で書く。ターゲットが `$(VAR)` のみで literal `.o`/`.mod` basename が残らない行は静的解析から除外され、依存欠落扱いとなる（`$(OBJDIR)/` 等の directory prefix は静的解析が basename へ正規化するため許容される）。
