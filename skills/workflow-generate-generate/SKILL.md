...
- `impl_defaults.toolchain.build_system=make` かつ `impl_defaults.toolchain.language=fortran` の場合、`src/Makefile` は `use` 依存に対応したオブジェクト依存関係を明示し、依存 `module` の `.mod` または依存 `.o` を各オブジェクトターゲットの前提条件へ必須記述する。オブジェクト規則のターゲット・前提条件はいずれも literal `.o`/`.mod` basename へ解決できる形（例: `foo.o:`、out-of-source なら `$(OBJDIR)/foo.o:`）で書く。`$(OBJDIR)/` 等の directory prefix は静的解析が basename へ正規化するため許容される。**前提条件を変数で書く場合**（例: `MODEL_OBJ = $(OBJDIR)/foo_model.o` を定義して `$(RUNNER_OBJ): foo_runner.f90 $(MODEL_OBJ)` のように参照する）も、静的解析が同一 Makefile 内の単純変数定義（`=` / `:=` / `?=` / `+=`）を展開して literal basename へ解決するため許容される。ただし変数は**当該規則より前**で定義すること（make は規則の前提条件を読み込み時に即時展開するため、後で定義する forward reference は空展開され依存欠落扱いとなる）。また定義が同 Makefile 内に無い未定義変数（展開不能で `.o`/`.mod` basename が残らない）を前提条件・ターゲットに使うと依存欠落扱いとなる。
