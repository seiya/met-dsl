...
- `impl_defaults.toolchain.language=fortran` の `runner` が論理値を JSON boolean として出力する際、literal `true` / `false` ではなく `L` 系 edit descriptor（`L1` 等、`T`/`F` トークンを生成する）を `diagnostics.json` / `perf.json` / `raw/metrics_basis.json` / `raw/state_snapshots/*.json` の boolean token へ直接使用する場合は `fail` とする。本検査は `validate_pipeline_semantics --stage post_generate` が runner source の静的解析で deterministic に強制する（descriptor・ファイル・行番号を報告）。
...
- `impl_defaults.toolchain.build_system=make` かつ `impl_defaults.toolchain.language=fortran` の場合、`src/Makefile` の各オブジェクトターゲットが `use` 依存に対応した `.mod` または依存 `.o` の前提条件を明示していることを検査する。規則行のターゲット・前提条件は literal `.o`/`.mod` basename へ解決できること（literal 名・`$(OBJDIR)/<basename>`・同 Makefile 内で**当該規則より前に**定義された単純変数を介した参照 `$(VAR)` のいずれか）を確認する。**変数を当該規則より後で定義した forward reference** は make が前提条件を空展開するため依存欠落扱いとする。同 Makefile 内に定義の無い未定義変数のみのターゲット・前提条件も依存欠落扱いとする。補足は `docs/RUNBOOK.md` の 1-2-1 を参照する。
