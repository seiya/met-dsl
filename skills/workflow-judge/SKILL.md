...
- `Judge` 開始前と完了前に `python3 tools/validate_pipeline_semantics.py --stage pre_judge` を実行し、`fail` 時は `Judge fail` とする。`--allow-missing-orchestration` と `--allow-missing-llm-review` を指定してはならない。
