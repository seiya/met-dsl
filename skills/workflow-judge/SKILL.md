...
- `raw` の必須構成は `derived_contract.json` の `raw_requirements.required_evidence` を正本として判定し、固定の証跡構成を一律必須にしてはならない。
...
- `Judge` 開始前と完了前に `python3 tools/validate_pipeline_semantics.py` を実行し、`fail` 時は `Judge fail` とする。
...
10. `python3 tools/validate_pipeline_semantics.py` の結果が `PASS` でない場合、`verdict.json` と `aggregate_verdict.json` を確定してはならない。
