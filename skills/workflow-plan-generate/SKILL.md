...
- 出力形式、入出力契約、判定条件の要求定義は `controlled_spec.md` と `tests.md` と `deps.yaml` と `algorithm.resolved.yaml` と `derived_contract.json` と `docs/` 正本を参照し、`tools/` 配下の検証 `python` スクリプトと `quality check` 実装を参照してはならない。
...
- `algorithm.resolved.yaml` は `Generate` の正本入力であり、`Generate` が `controlled_spec.md` を直接読まなくても実装可能な情報を保持しなければならない。
- `algorithm.summary.md` を自動生成し、閲覧専用成果物として保存する。
