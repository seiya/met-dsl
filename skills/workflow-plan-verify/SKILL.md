...
description: Plan ステージの verify を実行し、`case.resolved.yaml` と `algorithm.resolved.yaml` と `impl.resolved.yaml` と `dependency.resolved.yaml` の整合性、決定性、既定値適用を検査するときに使用する。Plan 生成後の `verification_status` 判定に適用する。
---
