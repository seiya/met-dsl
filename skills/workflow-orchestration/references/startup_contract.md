...
## Step 0 — TMPDIR セットアップ（必須先頭ステップ）

orchestration agent と子 agent の **両方が**、最初の `Bash` を実行する**前に** `TMPDIR` を `output_manifests/<agent_run_id>.json#allowed_tmp_root` から export しなければならない。これを怠ると、後続の `cat > $TMPDIR/x.patch <<EOF` 等が `output_manifest_write_guard` でブロックされ、復旧に複数往復を要する。

```bash
export TMPDIR=$(jq -er '.allowed_tmp_root' \
  "workspace/orchestrations/$METDSL_ORCHESTRATION_ID/output_manifests/<agent_run_id>.json")
```

- `<agent_run_id>` は orchestration agent では `orchestration_meta.json#orchestration_agent_run_id`、子 agent では `record-launch` 時に発行した `agent_run_id`。
- 以降の **すべて** の `cat > ...`、`mkdir -p ...`、`tee ...`、heredoc 書き込みは `$TMPDIR/` 配下で行う。
- TMPDIR 外への直接書き込み（`workspace/<canonical>/...` など）は `guarded-apply-patch` 経由を必須とし、`Edit`/`Write` tool は `allowed_file_tool_paths` 登録済みパスにのみ使用する。Bash heredoc で canonical path に直接書くと `enforce_guarded_apply_patch` でブロックされる。
...
1. agent_run_id (UUID) を生成する
   - canonical 経路: `CHILD_ARID=$(python3 tools/new_agent_run_id.py)`
...
5.4. record-child-return で Agent tool return 観測の証跡を残す（Adv-20/Adv-30 ガード必須）
     RETURN_TOKEN=$(cat <repo_root>/workspace/orchestrations/<orchestration_id>/launches/<agent_run_id>.parent_return_token)
...
コマンドを構築するときは下記テンプレートをそのままコピーして値を埋める。CLI 全 subcommand の完全 reference は `docs/CLI_REFERENCE.md` を canonical source とし、本テンプレートと併せて参照すれば `--help` を呼ぶ必要は無い (呼んでも害は無いが、本 file と CLI_REFERENCE.md だけで起動要件を満たせる構成にしている)。
