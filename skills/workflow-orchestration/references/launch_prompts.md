...
- For `.json` and `.txt` output, run `python3 tools/orchestration_runtime.py guarded-apply-patch --repo-root <repo_root> --orchestration-id <orchestration_id> --actor-role {{ACTOR_ROLE}} --agent-run-id <agent_run_id> --paths-json '["..."]' --patch-text '<patch_text>' --capability-token <capability_token>` as the only path (see the full procedure in docs/AGENT_CONTRACT.md), and stop editing on rejection.
...
- **verify-family substep (`compile.verify` / `generate.verify`): a `pass` requires `apply_patch_writes` gate evidence.** You MUST re-author the verified meta (`ir_meta.json` for compile, `source_meta.json` for generate) via `guarded-apply-patch` even when the inspection finds nothing to change (refresh an idempotent field such as `verify_attempts`). An inspect-only verify that emits no `guarded-apply-patch` write is rejected by `record-agent-run` and cannot terminate `pass`.
{{COMMON_BOILERPLATE}}
...
> The `.json` / `.txt` `guarded-apply-patch` write procedure, its forbidden-pattern (NG) examples, and the strip note now live in the child-readable canonical doc [docs/AGENT_CONTRACT.md](../../../docs/AGENT_CONTRACT.md) (§"`.json` / `.txt` artifact write"). The child reaches it via the AGENT_CONTRACT pointer expanded into the template body, so it is no longer inlined here.
...
The write of a `.json` / `.txt` patch file is canonically via the `Write` tool (see the "`.json` / `.txt` artifact write" section of [docs/AGENT_CONTRACT.md](../../../docs/AGENT_CONTRACT.md)).
