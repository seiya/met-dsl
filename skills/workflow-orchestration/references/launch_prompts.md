...
- **MCP `command_log.jsonl` & runner program-output placement (phase-scoped — skip the clauses that do not apply to your substep):** *If your substep runs a `build-runtime` MCP tool* (only `Generate.generate`'s `run_linter` among LLM leaves), its `command_log.jsonl` belongs in `allowed_output_paths` at the canonical per-phase path — already enumerated in your manifest and auto-injected by `record-launch`, so you need not add it; it is MCP-written, never write it with a file tool (`docs/AGENT_CONTRACT.md` §"MCP command_log placement"). *If your substep authors, verifies, or judges runner program output* (`Generate.generate` / `Generate.verify` / `Validate.judge`), the `diagnostics.json` / `perf.json` / `raw/*` contract is [docs/workflow/RUNNER_OUTPUT_CONTRACT.md](docs/workflow/RUNNER_OUTPUT_CONTRACT.md) — and those substeps force-read it (it is in their `skill_must_read_refs`). Other substeps (e.g. `Compile.*`) have neither contract and can ignore this line. The exhaustive per-phase placement / `src/Makefile` auto-inject / cross-phase Make-rule / `run_id` literal form / `command_log_ref`-trust reference is [docs/workflow/MCP_COMMAND_LOG_PLACEMENT.md](docs/workflow/MCP_COMMAND_LOG_PLACEMENT.md), readable under `docs/` but **not** force-read. (The pipeline `lineage.json` is NOT a leaf output — the conductor authors it host-side.)
