...
- **Slim warm-resume repair turn (`Generate.lint`/`Generate.static` finding only).** When a `reuse` repair warm-`--resume`s the producer session (claude, `METDSL_CONDUCTOR_REUSE_SLIM_PROMPT=1`), the conductor renders a distinct **slim** body — sentinel `Warm-resume slim repair turn`, the rotated new ids/source/output paths, and the lint/static `failure_excerpt` to fix — and omits the SKILL/must-read/boilerplate the resumed leaf already holds (like the deterministic prompt, it is built directly, not from this template, and the launch validator applies a reduced marker set). Canonical: [docs/design/deterministic_followups.md](docs/design/deterministic_followups.md) "G1-slim".
