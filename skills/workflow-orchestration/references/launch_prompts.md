...
- **MCP `command_log.jsonl` & runner program-output placement (phase-scoped — skip the clauses that do not apply to your substep):** *No LLM leaf runs a `build-runtime` MCP tool* — `run_linter` (`generate.lint`), `compile_project` (`build`) and `run_program` (`validate.execute`) are all deterministic conductor substeps. If a `command_log.jsonl` appears in your `allowed_output_paths` it is MCP/conductor-written at the canonical per-phase path and auto-injected by `record-launch`; never write it with a file tool (`docs/AGENT_CONTRACT.md` §"MCP command_log placement"). *If your substep authors, verifies, or judges runner program output* (`Generate.generate` / `Generate.verify` / `Validate.judge`), the `diagnostics.json` / `perf.json` / `raw/*` contract is [docs/workflow/RUNNER_OUTPUT_CONTRACT.md](docs/workflow/RUNNER_OUTPUT_CONTRACT.md) — and those substeps force-read it (it is in their `skill_must_read_refs`). Other substeps (e.g. `Compile.*`) have neither contract and can ignore this line. The exhaustive per-phase placement / `src/Makefile` auto-inject / cross-phase Make-rule / `run_id` literal form / `command_log_ref`-trust reference is [docs/workflow/MCP_COMMAND_LOG_PLACEMENT.md](docs/workflow/MCP_COMMAND_LOG_PLACEMENT.md), readable under `docs/` but **not** force-read. (The pipeline `lineage.json` is NOT a leaf output — the conductor authors it host-side.)
...
- **Slim warm-resume repair turn (`Generate.lint`/`Generate.static`/`Compile.static` finding).** When a `reuse` repair warm-`--resume`s the producer session (claude; always-on, no env gate), the conductor renders a distinct **slim** body — sentinel `Warm-resume slim repair turn`, the rotated new ids/source/output paths, and the lint/static/compile_static `failure_excerpt` to fix — and omits the SKILL/must-read/boilerplate the resumed leaf already holds (like the deterministic prompt, it is built directly, not from this template, and the launch validator applies a reduced marker set). Canonical: [docs/design/deterministic_followups.md](docs/design/deterministic_followups.md) "G1-slim".
