...
- Write every output artifact — managed JSON (`lineage.json` / `*_meta.json` / `verdict.json` / `diagnostics.json` …), source code, and `.yaml` / `.yml` / `.md` — directly with the `Edit` / `Write` tool, only to a path enumerated in the `allowed_file_tool_paths` of `output_manifests/<agent_run_id>.json`. Do not use `guarded-apply-patch`. The MCP-owned `mcp_command_log.jsonl` is written only by the build-runtime MCP server and must never be written with a file tool.
