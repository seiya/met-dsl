...
- `workflow-launch-check` が `status=fail_closed` を返した場合、`python3 tools/orchestration_runtime.py set-status --repo-root <repo_root> --orchestration-id <orchestration_id> --status fail_closed --reason-code <reason_code>` を実行して停止し、phase artifact を生成してはならない。`set-status` の同一 terminal status 再呼び出しは **idempotent** であり、cleanup_committed marker 未書き込みなら cleanup retry を実行、marker 書き込み済みなら no-op replay (response loss 後の defensive retry 想定) として受理される。narrative fields (`reason_code` / `reason_detail` / `blocking_policy_scope`) は 1 回目で確定し再呼び出しでは上書きされない。失敗内容の追記は `workspace/orchestrations/<orchestration_id>/failure_analysis.json` への直接編集で行う。terminal 間遷移は `fail` → `fail_closed` のみ許容され、他は runtime guard で reject される。
...
- `Validate execute` / `Validate judge` の起動要求では、`skill_must_read_refs` に `ir_ref` 配下の `spec.ir.yaml`、対象 `pipeline_ref` 配下の `source/<source_id>/source_meta.json`、`binary/<binary_id>/binary_meta.json` を必須記録しなければならない。`<binary_id>` と `<source_id>` の解決ルールは substep ごとに異なる:
  - `Validate execute`: launch request が `source_binary_id` (= `<binary_id>`) と `source_id` を必須記録する (runtime enforce 済み)。`binary_meta.json.source_source_id == launch_request.source_id` の一致も runtime が verify する。
  - `Validate judge`: launch request は `run_id` のみ必須 (`source_id` / `source_binary_id` は必須ではない)。judge は同一 `run_id` 配下の `trial_meta.json` を必ず読み、`trial_meta.json.source_source_id` を `<source_id>` として、`trial_meta.json.source_binary_id` を `<binary_id>` として解決する (trial_meta は execute が書き込み、runtime が binary_meta との一致を verify 済みなので、retry で `pipeline_id` 配下に複数の source / binary が共存しても judge が誤った id を読む経路は無い)。
...
- `workflow-launch-check` の dependency readiness 判定を通すため、`orchestration_meta.json.dependency_readiness` に `direct_dependency_compile_readiness` と `direct_dependency_execution_readiness` と `detail.ir_ref_verified` と `detail.pipeline_ref_verified` と `detail.aggregate_verdict_verified` を必須記録する。未記録または未充足の場合、`workflow-launch-check` は `fail_closed` を返す。leaf node (deps.yaml の依存が空) は preflight が自動で trivial-true に設定する。非自明な依存を持つ node は、依存 artifact (`workspace/ir/<dep>/.../ir_meta.json`、`workspace/pipelines/<dep>/.../binary_meta.json`、`workspace/pipelines/<dep>/.../aggregate_verdict.json`) を生成・検証した後に `python3 tools/orchestration_runtime.py mark-dependency-readiness --repo-root <repo_root> --orchestration-id <orchestration_id>` を実行する。CLI は assertion ではなく runtime 検証要求であり、毎回 workspace 上の **canonical id 順 (`<slug>_<YYYYMMDD>_<seq3>` の `(date, seq)`) で選んだ artifact 実態** (`verification_status=pass`、または `aggregate_verdict ∈ {pass, xfail}`) と version_constraint resolve 結果を inspect して全 flag を再計算・上書きする (mtime は信用しない: Codex round 23 F1; 同一 `(date, seq)` の slug 違い衝突は fail-closed: Codex round 35 F1)。`dep_set_fingerprint` (spec_ref + deps.yaml の SHA-256) も毎回更新され、preflight 再実行時に fingerprint mismatch が検出されると persisted flag は全リセットされる。artifact 未生成・verification_status≠pass・version_constraint 解決不能・spec_ref 差し替えのいずれでも flag は false のまま (gate bypass 防止)。
...
18. workflow 終了時は `python3 tools/orchestration_runtime.py set-status --repo-root <repo_root> --orchestration-id <orchestration_id> --status <status>` を実行し、`orchestration_meta.json` を終端状態へ更新する。narrative fields (`reason_code` / `reason_detail` / `blocking_policy_scope`) は 1 回目の呼び出しで確定する。同一 terminal status の再呼び出しは **idempotent** で受理 (cleanup_committed marker 未書き込みなら cleanup retry、marker 書き込み済みなら no-op replay) され、narrative は上書きされない。narrative の追加・修正は `failure_analysis.json` を編集する。terminal 間遷移は `fail` → `fail_closed` のみ許容され、他は runtime guard により reject される。
...
- CLI 仕様 reference (場所別最適化、詳細は `CLAUDE.md` の「CLI 仕様の確認規約」節):
  ...
  - 稀少 subcommand (`init` / `preflight` / `preflight-status` / `record-timeout` / `read-checkpoint` / `verify-checkpoint-integrity` / `check-step-completed` / `orchestration-read` / `repair-agent-runs`): `python3 tools/orchestration_runtime.py <sub> --help`。overview は `docs/CLI_REFERENCE_RARE.md`
