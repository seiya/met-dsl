...
- `Plan` phase の `generate` substep を起動するとき、`dependency_ref` には `spec/<component_path>/deps.yaml` 形式の実在 path を必ず渡さなければならない。`dependency_ref` が空または placeholder の場合は起動してはならない。`deps.yaml` の実在確認は `run-gate --gate orchestration_read` を使用すること。
...
8. `record-launch` 実行前に、`plan_ref` と `pipeline_ref` と `dependency_ref` に placeholder が残存していないこと、`step=plan` の substep 起動要求の `dependency_ref` が `spec/` で始まり `/deps.yaml` で終わる文字列であること（非空・非 placeholder を確認）、`verify` 起動要求の `skill_must_read_refs` が必須 resolved artifact を網羅していること、起動要求本文に non-canonical な `tools/` / validator 参照禁止が含まれていることを検査する。
