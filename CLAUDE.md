...
6. deactivate-child を実行して active context を orchestration agent へ切り戻す
7. record-reply で launches/<agent_run_id>.reply.txt に応答テキストを上書き保存する
8. record-agent-run を実行して agent_runs.jsonl へ追記する
```
...
- `launches/<agent_run_id>.reply.txt` は手順 3 で record-launch が暫定書き込みし、手順 7 の record-reply で Agent tool の実際の応答テキストに上書きする。
