...
- The canonical entrypoint for starting the workflow is `python3 tools/run_workflow.py <spec_ref> <until_phase> [--llm <codex|cursor|claude>]`. `<until_phase>` specifies one of `compile` / `generate` / `build` / `validate`. By default this targets a **single node**; a node whose dependencies are not yet present in the workspace is fail-closed (`dependency_not_ready`) and the operator runs each dependency node first. Adding `--with-deps` makes the entrypoint resolve the target's transitive dependency closure and run each not-yet-ready dependency node bottom-up (dependency order, one orchestration per node) before the target — dependency nodes run to Compile when `<until_phase>=compile`, else to Validate. `--with-deps` is ignored with `--resume` (target only).
- The orchestration driver is selected with `--orchestrator {llm,conductor}`. The default is `conductor` for the `claude` / `codex` backends and `llm` for `cursor` (the conductor has no `cursor` leaf launcher). `conductor` drives the deterministic phase/substep loop in Python (`tools/workflow_conductor.py`), invoking the LLM only as a leaf for each substep body — this removes the parent orchestration LLM's per-turn `cache_read` overhead. `llm` spawns an LLM orchestration agent to drive the phase loop instead (this file's per-turn launch conventions describe that path; pass `--orchestrator llm` explicitly to use it). See [docs/design/deterministic_conductor.md](docs/design/deterministic_conductor.md). (Status: conductor deterministic loop + failure routing + LLM escalation landed; the real `claude -p` MCP/hooks path is verified only by mocked unit tests, not yet by a live integration run — pass `--orchestrator llm` to fall back if a live conductor run misbehaves.)
- The canonical path for resuming a workflow that failed midway is `python3 tools/run_workflow.py --resume [--orchestration-id <id>]`. With `--resume`, `spec_ref` / `until_phase` / `--llm` / `--mode` / `--orchestrator` can be omitted and are restored from the target orchestration's existing artifacts (`orchestration_meta.json` / `preflight.json` / `launches/orchestration.start.prompt.txt`; the orchestration driver is restored from the `orchestrator.json` marker, which both drivers write — a missing marker means a legacy LLM-driven run) (an explicit argument takes precedence; when `--orchestration-id` is omitted, the most recent one in chronological order is targeted). Internally it runs `init --resume-from-checkpoint` (`resume_enabled=true`, resetting a terminal status to `running`), and together runs `repair-agent-runs` to backfill the `parent_agent_run_id` / `agent_model` missing from pre-`caa10ab` legacy rows of `agent_runs.jsonl` from the authoritative source (if `agent_model` cannot be auto-derived it becomes `needs_manual`, and the operator runs `repair-agent-runs --agent-model <id>` manually). For details, refer to [docs/RUNBOOK.md](docs/RUNBOOK.md) §3-1.
...
6. Run **finalize-child** to finalize in ONE call — it performs, in order and with every guard,
   record-child-return → deactivate-child → record-reply → record-agent-run. One call instead of
   four keeps the orchestration transcript (and its per-turn cache-read cost) small.
   → required arguments: `--agent-run-id`, `--return-token "<literal token>"` (Adv-30 parent-bound
     token; auto-generated by record-launch), `--reply-text "<terse Agent return>"`, and
     `--agent-run-json '<record-agent-run payload>'` (its `agent_run_id` must equal `--agent-run-id`)
   → pass the token via the **two-step method** (steps 6a → 6b; see "`parent_return_token` reference
     convention" below). Do not use the `$(cat ...)` command-substitution form because it does not
     pass the Bash tool's static analysis.
   → keep `--reply-text` terse: a `status:` line, `output_refs:`, and a few lines of rationale —
     full detail lives in the child's artifacts. record-agent-run flags an over-budget reply
     (`reply_over_budget`) and rejects it when `METDSL_ENFORCE_REPLY_BUDGET=1`.
   → for edge recovery the four individual subcommands remain available and must run in that order.
```

**Necessity of running steps 4–6 contiguously (active_child window):** Launching the `Agent` tool in step 4 switches the active context to the child, and it remains the child until `deactivate-child` (run internally by `finalize-child` in step 6). **During this period (from the Agent tool return in step 5 until step 6 completes), the parent orchestration agent must not issue any file write via `Write` / `Edit` / `Bash`.** If it does, even a write to the parent's own `workspace/tmp/<self_arid>/` is evaluated against the child's `output_manifest` and rejected by `output_manifest_write_guard` (`agent_run_id=<child>`) (this hook block has been observed in past audits). Run steps 4 through 6 contiguously without interruption, and batch the parent's own tmp scripts or arbitrary-path writes after step 6 completes or just before step 3 (`record-launch`).
...
- **Step 6b (pass):** embed the token printed in 6a as a **literal string** into `finalize-child --return-token "<literal token>"` and run it.
...
- `launches/<agent_run_id>.reply.txt` is provisionally written by record-launch in step 3, and overwritten with the actual Agent tool response text by record-reply (run inside finalize-child) in step 6.
