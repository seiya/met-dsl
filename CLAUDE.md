...
- The orchestration driver is selected with `--orchestrator {llm,conductor}` (default `llm`). `llm` spawns an LLM orchestration agent to drive the phase loop (this file's conventions describe that path). `conductor` drives the deterministic phase/substep loop in Python (`tools/workflow_conductor.py`), invoking the LLM only as a leaf for each substep body — this removes the parent orchestration LLM's per-turn `cache_read` overhead. See [docs/design/deterministic_conductor.md](docs/design/deterministic_conductor.md). (Status: conductor happy-path + failure routing landed; the real `claude -p` MCP path and the LLM diagnostician are pending, so `llm` remains the default.)
