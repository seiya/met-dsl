...
- For the rare subcommands of `tools/orchestration_runtime.py` (`init` / `preflight` / `preflight-status` / `record-timeout` / `read-checkpoint` / `verify-checkpoint-integrity` / `check-step-completed` / `orchestration-read` / `repair-agent-runs` / `repair-step-result-executor` / `reopen-phase` / `dismiss-violation`), and for `tools/run_workflow.py` / `tools/validate_pipeline_semantics.py` / `tools/audit_orchestration.py`, `<tool> [<sub>] --help` is the canonical source. `docs/CLI_REFERENCE_RARE.md` retains only an overview of the rare subcommands.
